...
static bool parse_algo(const char *arg);
...
struct work {
	...
	unsigned char ntime[4]; // Add ntime field needed for stratum mining
};
...
static void *miner_thread(void *userdata)
{
	...
	while (1) {
		...
		work.data[20] = 0x00000000;

		if (have_stratum) {
			uint32_t ntime, nonce;

			work.data[19] = le32dec(work.data + 19) + 1;
			ntime = le32dec(work.ntime);
			nonce = le32dec(work.data + 19);
			/* check if ntime needs to be updated */
			if (ntime != work.data[17]) {
				work.data[17] = ntime;
				/* update header */
				if (opt_algo == ALGO_SHA256D) {
					sha256d_preextend(work.data);
					if (opt_debug)
						applog(LOG_DEBUG, "stratum: updated header with ntime %08x",
							ntime);
				}
			}
			hashes_done = 0;
			...
			if (unlikely(rc < 0)) {
				applog(LOG_ERR, "scanhash error: %d, exiting "
				       "mining thread %d", rc, thr_id);
				goto out;
			}

			hashes_run = rc > 0 ? rc : hashes_done;
			work.data[19] = nonce + hashes_run;
			...
			/* extract hashrate before work update so it matches the work we were doing, not the new one */
			if (!opt_quiet) {
				pthread_mutex_lock(&stats_lock);
			...
			// Check if need to scan to full range
			max64 = end_nonce;
			max64 *= opt_n_threads;
		...
		if (!have_stratum) {
			int64_t nonce_range, max_nonce_diff;
			int scan_time = opt_scantime;

			nonce_range = (int64_t)max_nonce - (int64_t)work.data[19] - 1;
			if (nonce_range <= 0)
				nonce_range = 1;
			max_nonce_diff = ((int64_t)scan_time * (int64_t)hashes_done) / (diff.tv_sec + diff.tv_usec / 1e6);
	
			if (max_nonce_diff > nonce_range) {
				max_nonce_diff = nonce_range;
			}
			if (max_nonce_diff < 1)
				max_nonce_diff = 1;
			work.data[19] += max_nonce_diff;
			...
			if (opt_benchmark && thr_id == opt_n_threads - 1) {
				...
				for (i = 0; i < opt_n_threads && thr_hashrates[i]; i++)
					hashrate += thr_hashrates[i];
				if (i == opt_n_threads) {
					applog(LOG_INFO, "Total: %.2f khash/s",
							hashrate / 1000.0);
				}
			}
		}
...
// Forward declarations - add near the top of file after includes
static bool parse_algo(const char *arg);

// Define parse_algo function early, before it's used
static bool parse_algo(const char *arg)
{
	if (!strcasecmp(arg, "sha256d")) {
